"""
Stage 2：LatentDiffusion 精调（加载 Stage 1 权重）
用法: python stage2.py config.yaml --pretrained ./logs/stage1/checkpoints/best.pt --logdir ./logs/stage2
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Stage 2 光谱补全训练")
...
def main(args):
    ...
    def train(current_step):
        ...
        out = model(batch, known_mask, use_diffusion=True, use_decoder=True)
        ...
        if current_step % config.log_freq == 0:
            ...
            logger.info(f"[stage{stage} train] Iter {current_step:06d}/{config.max_iters} " + loss_str)
    ...
    def validate(current_step):
        ...
        with torch.no_grad():
            ...
            logger.info(f"[stage{stage} val]   Iter {current_step:06d}/{config.max_iters} " + loss_str)
