"""
Stage 3：冻结 Encoder，精调 Decoder（加载 Stage 2 权重）
用法: python stage3.py config.yaml --pretrained ./logs/stage2/checkpoints/best.pt --logdir ./logs/stage3
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Stage 3 光谱补全训练")
...
def main(args):
    ...
    if not resume and args.pretrained is not None:
        logger.info(f"[Stage {stage}] 加载预训练权重: {args.pretrained}")
        ckpt_pre = torch.load(args.pretrained, map_location="cpu")
        missing, unexpected = model.load_state_dict(ckpt_pre["model"], strict=False)
    ...
    use_lr_schedule = config.lr_schedule.use
    high_lr_scale = config.lr_schedule.high_lr_scale
    high_lr_steps = config.lr_schedule.high_lr_steps
    base_lr = config.lr
    initial_lr = base_lr * high_lr_scale if use_lr_schedule else base_lr

    optim = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=initial_lr)
    ...
    if use_lr_schedule:
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            ...
            threshold=config.lr_schedule.threshold,
            threshold_mode=config.lr_schedule.threshold_mode,
            cooldown=config.lr_schedule.cooldown,
            min_lr=config.lr_schedule.min_lr,
            eps=config.lr_schedule.eps,
        )
        logger.info(
            f"[Stage {stage}] LR调度: 初始LR={initial_lr:.6e}, 基础LR={base_lr:.6e}, "
            f"高LR步数={high_lr_steps}, Plateau(factor={getattr(config.lr_schedule, 'factor', 0.5)}, "
            f"patience={getattr(config.lr_schedule, 'patience', 3)}, min_lr={getattr(config.lr_schedule, 'min_lr', 1e-6)})"
        )
    ...
    def train(current_step):
        nonlocal train_iter
        model.eval()
        for mod_name in train_modules: getattr(model, mod_name).train()

        if use_lr_schedule and high_lr_steps > 0 and current_step == high_lr_steps + 1:
            for group in optim.param_groups:
                group["lr"] = base_lr
            logger.info(f"[Stage {stage}] Iter {current_step}: 切换到基础LR={base_lr:.6e}")

        train_iter, batch = next_batch(train_iter, train_loader, args.device)
        known_mask = sample_known_mask(
            batch_size=config.batch_size, num_modalities=len(config.modality_names),
        ...
        if not torch.isfinite(out[primary_key]):
            logger.warning(f"[Stage {stage}] Iter {current_step}: {primary_key}=NaN，跳过本步")
        ...
        if "cos_sims" in out:
            ...
            writer.add_scalar("train/cos_sims_mean", cos_vals.mean().item(), current_step)
        ...
        writer.add_scalar("train/lr", optim.param_groups[0]["lr"], current_step)
        writer.flush()
    ...
    def validate(current_step):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"[Stage {stage}] Val"):
                ...
                known_mask = sample_known_mask(
                    batch_size=batch["enc_batch"][config.modality_names[0]].size(0), num_modalities=len(config.modality_names),
                    drop_prob=config.drop_prob, device=args.device
                )
        ...
        for current_step in range(start_iter, config.max_iters + 1):
            ...
            if current_step % config.val_freq == 0 or current_step == config.max_iters:
                ...
                if scheduler is not None:
                    ...
                    writer.add_scalar("val/lr", new_lr, current_step)
                    if new_lr < prev_lr:
                        logger.info(f"[Stage {stage}] Iter {current_step}: 验证集无提升，LR {prev_lr:.6e} -> {new_lr:.6e}")
...
def test(args):
    config_path = glob(os.path.join(args.config, "*.yaml"))[0]
    with open(config_path, "r", encoding="utf-8") as f: config = EasyDict(yaml.safe_load(f))
    stage = int(config.stage)
    seed_all(config.seed)
    ...
    fused_stats_csv = os.path.join(log_dir, "fused_mu_mask_stats.csv")
    mus_logvars_stats_csv = os.path.join(log_dir, "mus_logvars_mask_stats.csv")
    mus_logvars_fieldnames = ["batch_idx", "known_mask_pattern"]
    for name in config.modality_names:
        mus_logvars_fieldnames.extend(
            [
                f"mu_mean_{name}",
                f"mu_std_{name}",
                f"mu_l2_mean_{name}",
    ...
    with open(fused_stats_csv, "w", encoding="utf-8", newline="") as f_stats:
        fused_stats_writer = csv.DictWriter(
            f_stats,
            fieldnames=[
                "batch_idx",
                "known_mask_pattern",
        ...
        fused_stats_writer.writeheader()
        with open(mus_logvars_stats_csv, "w", encoding="utf-8", newline="") as f_mus_logvars:
            mus_logvars_writer = csv.DictWriter(f_mus_logvars, fieldnames=mus_logvars_fieldnames)
            mus_logvars_writer.writeheader()
            with open(stats_csv, "w", encoding="utf-8", newline="") as f_csv:
                ...
                with torch.no_grad():
                    for i, batch in enumerate(tqdm(test_loader, desc=f"[Stage {stage}] Test")):
                        batch = move_to_device(batch, args.device)
                        known_mask = sample_known_mask_test(batch_size=batch["enc_batch"][config.modality_names[0]].size(0), num_modalities=M, missing_modalities=missing_modalities, device=args.device)
                        spec_pred, spec_missing_loss, cos_sims, debug = model.predict_spectra(batch, known_mask=known_mask, use_diffusion=args.use_diffusion_infer, return_debug=True)
                        ...
                        fused_mu = debug["fused_mu"].to(torch.float32)
                        fused_logvar = debug["fused_logvar"].to(torch.float32)
                        fused_stats_writer.writerow(
                            {
                                "batch_idx": i,
                                "known_mask_pattern": repr(known_mask[0].tolist()),
                                "fused_mu_mean": float(fused_mu.mean().item()),
                                "fused_mu_std": float(fused_mu.std(unbiased=False).item()),
                                "fused_mu_l2_mean": float(torch.norm(fused_mu, dim=1).mean().item()),
                                "fused_logvar_mean": float(fused_logvar.mean().item()),
                                "fused_logvar_std": float(fused_logvar.std(unbiased=False).item()),
                            }
                        )

                        mus = debug["mus"].to(torch.float32)
                        logvars = debug["logvars"].to(torch.float32)
                        mus_logvars_row = {
                            "batch_idx": i,
                            "known_mask_pattern": repr(known_mask[0].tolist()),
                        }
                        for m_idx, name in enumerate(config.modality_names):
                            mu_m = mus[:, m_idx]
                            logvar_m = logvars[:, m_idx]
                            mus_logvars_row[f"mu_mean_{name}"] = float(mu_m.mean().item())
                            mus_logvars_row[f"mu_std_{name}"] = float(mu_m.std(unbiased=False).item())
                            mus_logvars_row[f"mu_l2_mean_{name}"] = float(torch.norm(mu_m, dim=1).mean().item())
                            mus_logvars_row[f"logvar_mean_{name}"] = float(logvar_m.mean().item())
                            mus_logvars_row[f"logvar_std_{name}"] = float(logvar_m.std(unbiased=False).item())
                        mus_logvars_writer.writerow(mus_logvars_row)
